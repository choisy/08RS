...
if (Sys.getenv("USER") == "MarcChoisy" | Sys.getenv("USER") == "marc") {
  ...
  data_folder  <- paste0(OneDrive, "SharePoint/OUCRU Modelling/08RS/")
...
children <- selection08RS |> 
  ...
  mutate(across(contains("HFMD"), ~ c(1:6, 0)[.x] + 1),                # recode grade
         HFMD = max(across(c(HFMDToday, HFMDAdmitted, GradeHFMD)), na.rm = TRUE)) |>
  ungroup() |> 
  recoding1() |> 
  recoding2() |>
...
# undoing the change on the dictionary done at the top of the chunk
CRF_dict <- CRF_dict0
rm(CRF_dict0)
```
