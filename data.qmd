...
lr_out <- expand_grid(response  = c("CS", "RC", "EC", "FM", "GM"),
                      timepoint = c("enrollment", "6 months", "18 months")) |> 
  mutate(dataset = map2(response, timepoint,
                        ~ make_data(.x, .y) |> 
                          mutate(across(group, ~ .x == "HFMD"),
                                 ps = logistic_ps(pick(everything()), ps_formula),
                                 ow = ifelse(group, 1 - ps, ps))),
         ctable = map(dataset, ~ .x |>
                        ...
                        table() |>
                        as.vector() |>
                        setNames(c("control", "HFMD")))) |> 
  unnest_wider(ctable) |>
  mutate(balance = map(dataset, ~ bal.tab(ps_formula, data = .x, s.d.denom = "treated",
                                          weights = "ow", un = TRUE)),
         pass = all(abs(a$Balance$Diff.Adj) < .1),
         model = map2(dataset, response, ~ .x |>
                       svydesign(ids = ~1, weights = ~ow, data = _) |> 
                       svyglm(reformulate("group", .y), design = _)),
         coef = map_dbl(model, ~ coef(.x)[2]),
         ci = map(model, ~ confint(.x)[2, ])) |> 
  unnest_wider(ci) |> 
  mutate(p = map_dbl(model, ~ last(as.vector(coefficients(summary(.x))))))
```

TO DO: filter (1) the tables that have 1 value and (2) the group that have 3 values
...
```{r}
lr_out2 <- expand_grid(response  = c("MD1ISS", "MD2ISS", "MD3ISS", "AC1ISS", "AC2ISS",
