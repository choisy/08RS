...
(patch <- selection02EI |> 
  ...
  mutate(across(ParNo, ~ with(ID_conv, setNames(s08RS, s02EI))[.x])))
```

Patching the data values to the `children` data frame:

```{r}
tmp <- filter(children, ParNo %in% ID_conv$s08RS)
tmp_names <- names(tmp)
tmp <- bind_cols(patch, tmp[, setdiff(tmp_names, names(patch))])[tmp_names]

children <- children |>
  filter(! ParNo %in% ID_conv$s08RS) |> 
  bind_rows(tmp) |> 
  arrange(ParNo)
```
...
```{r}
heatmap2 <- function(x, nbrow = 30, nb_wc = 3,
