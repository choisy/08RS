...
### Propensity scores v2

Compared to the version 1 the v2 allows to specify the covariables we want to include.
In doing so we can redefine the groups (useful to )
...
```{r}
make_data2 <- function(data, response, timepoint, covariab = covariables) {
  data |> 
    filter(time_point == timepoint) |> 
    select(ParNo, all_of(response)) |> 
    na.exclude() |> 
...
```{r}
prepare_fu_data2 <- function(data = followups, min_nb = 4, covariab = covariables) {
  expand_grid(response  = scores,
              timepoint = c("enrollment", "6 months", "18 months")) |> 
...
```{r}
pipeline2 <- function(data = followups, min_nb = 4, covariab = covariables) {
  data |> 
