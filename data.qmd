...
A tuning of the `readxl::excel_sheets()` function:

```{r}
excel_sheets2 <- make_data_reader(readxl::excel_sheets)
```

A tuning of the `readxl::read_excel()` function:

```{r}
read_excel2 <- make_data_reader(readxl::read_excel)
```

A tuning of the `readr::read_csv()` function:

```{r}
read_csv2 <- make_data_reader(readr::read_csv)
```
...
read_excel_file <- function(file) {
  sheets_names <- excel_sheets2(file)
  sheets_names |>
...
`df`, using the `key` variable as a common key between the two data frames:

```{r}
patch <- function(patch_df, df, key) {
  ref <- df[, key]
  sel <- df[[key]] %in% patch_df[[key]]
  tmp <- df[sel, ]
  tmp_names <- names(tmp)
  tmp <- bind_cols(patch_df, tmp[, setdiff(tmp_names, names(patch_df))])[tmp_names]
  df[! sel, ] |> 
    bind_rows(tmp) |> 
...
```{r fig_responses_distribution1, fig.width = 7, fig.height = 10}
...
```{r fig_responses_distribution1h, fig.width = 7, fig.height = 10}
opar <- par(mfrow = c(8, 4), plt = c(.2, .97, .27, .87))
output |>
  select(dataset, response, timepoint) |> 
...
```{r fig_responses_distribution2, fig.width = 7, fig.height = 10}
opar <- par(mfrow = c(8, 4), plt = c(.2, .97, .27, .87))
...
```{r fig_responses_distribution2h, fig.width = 7, fig.height = 10}
opar <- par(mfrow = c(8, 4), plt = c(.2, .97, .27, .87))
...
```{r include = FALSE}
if (file_exists("output_sev.rds")) {
  output_sev <- readRDScache("output_sev.rds")
} else {
  output_sev <- filter_patients(group == "control" |
                                  !(HFMD %in% c("grade 2a", "Not Applicable"))) |> 
    pipeline()
  saveRDScache(output_sev, "output_sev.rds")
}
```
...
```{r fig_Bayley_sev, fig.width = 7, fig.height = 25 / 3}
opar <- par(mfrow = c(5, 3), plt = c(.15, .97, .15, .9))
...
```{r include = FALSE}
if (file_exists("output_ev71.rds")) {
  output_ev71 <- readRDScache("output_ev71.rds")
} else {
  output_ev71 <- filter_patients(group == "control" | str_detect(PCR, "EV-A71")) |> 
    pipeline()
  saveRDScache(output_ev71, "output_ev71.rds")
}
```
...
```{r fig_Bayley_ev71, fig.width = 7, fig.height = 25 / 3}
opar <- par(mfrow = c(5, 3), plt = c(.15, .97, .15, .9))
...
```{r include = FALSE}
if (file_exists("output_sev71.rds")) {
  output_sev71 <- readRDScache("output_sev71.rds")
} else {
  output_sev71 <- filter_patients(group == "control" |
                                    (str_detect(PCR, "EV-A71") &
                                       !(HFMD %in% c("grade 2a", "Not Applicable")))) |> 
    pipeline()
  saveRDScache(output_sev71, "output_sev71.rds")
}
```
...
```{r fig_Bayley_sev71, fig.width = 7, fig.height = 25 / 3}
opar <- par(mfrow = c(5, 3), plt = c(.15, .97, .15, .9))
...
fu4 |> 
with(plot(jitter(enrollment), jitter(`6 months`)))
```
