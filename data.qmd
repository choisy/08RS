...
a <- followups |> 
  ...
  map2(list(convergence_glmmTMB, convergence_glm, convergence_survival),
       \(slot, fct) fct(slot)) |> 
  bind_rows() |> 
  arrange(order) |> 
...
a <- followups |> 
  ...
  group_split() |> 
  map2(list(
    function(df) mutate(df, formula = map(response, ~ reformulate(covariates2, .x))),
    function(df)
      df |>
        mutate(n = case_when(response == "BAL3" ~ 5, .default = 10),
               formula =
    ...
    function(df) {
      df |>
        mutate(formula = map(response,
                             ~ reformulate(covariates2, paste0("Surv(", .x, ")"))))},
    function(df) {
      df |> 
        mutate(right_value  = case_when(response == "BAL2" ~ 15, .default = 30),
               response_var = map2(dataset, response, ~ mextract2(.x, .y)),
               ...
               formula = map(response,
                             ~ reformulate(covariates2,
                                           paste0("Surv(", .x,
                                                  ", event = status)")))) |> 
        select(-c(right_value, response_var, status))}), \(slot, fct) fct(slot)) |> 
  bind_rows() |> 
  mutate(group = case_when(distr == "betabinomial" ~ "1",
                           distr == "gamma"        ~ "2",
                           .default                = "3")) |> 
  group_by(group) |> 
  group_split() |> 
  map2(list(
         function(df)
           mutate(df, mod = map2(formula, dataset,
                                 ~ glmmTMB(.x, .y, betabinomial))),
         function(df)
           mutate(df, mod = map2(formula, dataset,
                                 ~ glm(.x, Gamma(link = "log"), .y))),
         function(df)
           mutate(df,
                  across(dataset,
                         ~ map(.x,
                               \(x) mutate(x,
                                           across(1, ~ ifelse(.x == 0, 1e-16, .x))))),
                  mod = pmap(list(formula, dataset, distr),
                             \(x, y, z) survreg2(x, y, dist = z)))),
       \(slot, fct) fct(slot)) |> 
  map2(list(
         function(df) mutate(df, converged = map_lgl(mod, ~ .x$fit$convergence < 1)),
         function(df) mutate(df, converged = map_lgl(mod, "converged")),
         function(df) mutate(df, converged = map_lgl(mod, ~ compute_Q(.x) < 1e-6))),
       \(slot, fct) fct(slot)) |> 
  bind_rows() |> 
  arrange(order) |> 
  select(-c(group, order))
