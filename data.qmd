...
```{r}
required <- c("readxl", "purrr", "dplyr", "magrittr", "tidyr", "anthro")
```
...
information, and `followups` that contains the follow-up data. Let's look at the age
distribution of the controls and HFMD cases:
...
legend("topright", legend = c("control", "HFMD"), fill = adjcol(rev(cols)), bty = "n")
```
...
#  as.matrix() %>%
  `[`(edu_order, ) |> 
  t() |> 
  barplot(names.arg = c("none", "some", "5th grd", "9th grd", "12th grd", "college",
                        "postgrad"), beside = TRUE, col = adjcol(rev(cols)),
          ylab = "proportion")

legend("topleft", legend = c("control", "HFMD"), fill = adjcol(rev(cols)), bty = "n")
```
