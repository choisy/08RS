...
Comparing the missing values in the CRF and in Saras' CSV file:
...
cases |> 
  number_of_NA() |> 
  filter(variable %in% c("headCircumference", "height", "weigh", "stunting", "wasting",
                         "headCircumference_S", "HEIGHT", "WEIGHT", "stunt", "waste"))
```

Recomputing `stunting` and `wasting` from `WEIGHT` and `HEIGHT`:

```{r}
children <- children |> 
  stunt_waste(WEIGHT, HEIGHT) |> 
  select(-stunt, -waste)
```
