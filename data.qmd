  ...
  expand_grid(response  = c("MD1ISS", "MD2ISS", "MD3ISS", "AC1ISS", "AC2ISS",
    ...
    mutate(dataset = map(dataset,
                         ~ mutate(.x, ps = logistic_ps(pick(everything())),
                                      ow = ifelse(group == "HFMD", 1 - ps, ps),#logreg
                                      ow = ifelse(group == "HFMD", 1, ps / (1 - ps)),#logreg
                                      tw = twang_ps(pick(everything())),       #twang1
                                      t2 = twang_ps(pick(everything()),        #twang2
                                                    n.trees = 5000,
                                                    interaction.depth = 2))),
...
# 6 - TMLE:           
           tmle = map2_dfr(dataset, response, tmle2)) |> 
    unnest_wider(tmle)
}
```
...
```{r}
output2 <- output |> 
  mutate(dataset = map(dataset, ~ mutate(.x, ow2 = ifelse(group == "HFMD", 1, ps / (1 - ps)))),
         mdifow2 = map_dbl(dataset, get_max_diff, "ow2"),
         m_ow2 = map2(formula, dataset, ~ svyglm2(.x, .y, ~ow2)),
         cf_ow2 = map_dbl(m_ow2, coef2),
         ci = map(m_ow2, confint2)) |> 
  unnest_wider(ci) |> 
  rename_with(~ paste0(.x, "ow2"), ends_with("%")) |> 
  mutate(p_ow2 = map_dbl(m_ow2, get_p))
```

```{r}
output3 <- output2 |> 
  mutate(dataset = map(dataset, ~ mutate(.x, ow3 = logistic_ps2(pick(everything())))),
         mdifow3 = map_dbl(dataset, get_max_diff, "ow3"),
         m_ow3 = map2(formula, dataset, ~ svyglm2(.x, .y, ~ow3)),
         cf_ow3 = map_dbl(m_ow3, coef2),
         ci = map(m_ow3, confint2),
         mdifow2 = map_dbl(dataset, get_max_diff, "ow2")) |> 
  unnest_wider(ci) |> 
  rename_with(~ paste0(.x, "ow3"), ends_with("%")) |> 
  mutate(p_ow3 = map_dbl(m_ow3, get_p))
```
...
```{r}
output3 |> 
  select(response, timepoint, control, HFMD, starts_with("md")) |> 
